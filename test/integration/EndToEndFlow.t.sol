...
contract EndToEndFlowTest is TestSetup {
    ...
    function _executeCreatorRegistration(address newCreator, uint256 subscriptionPrice, string memory profileData) private returns (bool success) {
    ...
    function _executeContentPublication(address creator, uint256 price, string memory title) private returns (uint256 contentId) {
    ...
    function _executePaymentIntent(bytes32 intentId, address user, address paymentToken, uint256 tokenAmount, uint256 usdcEquivalent) private {
        vm.prank(operatorSigner);
        commerceIntegration.provideIntentSignature(bytes16(intentId), abi.encodePacked(bytes32("sub"), bytes32("signature"), bytes1(0x1b)));
    ...
    function _verifyPaymentIntentSuccess(PaymentIntentData memory intentData, CommerceProtocolIntegration.PaymentType expectedType) private view {
    ...
    function _getCreatorSubscriptionEarnings(address creator) private view returns (uint256 totalEarnings, uint256 withdrawable) {
    ...
    function _getCreatorPayPerViewEarnings(address creator) private view returns (uint256 totalEarnings, uint256 withdrawable) {
    ...
    function _verifyCreatorStats(address creator, uint256 expectedContentCount, uint256 expectedSubscriberCount) private view {
    ...
    function _verifyPlatformMetrics(
        uint256 expectedActiveSubscriptions,
        uint256 expectedTotalVolume
    ) private view {
        (
