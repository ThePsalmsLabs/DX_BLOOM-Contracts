...
contract CommerceProtocolFlowTest is TestSetup {
    ...
    function test_CreateContentPaymentIntent_AlternativeToken() public {
        ...
        CommerceProtocolIntegration.PaymentContext memory storedContext = commerceIntegration.getPaymentContext(intent.id);
    ...
    function test_SubscriptionFlow_AlternativeToken() public {
        ...
        SubscriptionManager.SubscriptionRecord memory record = subscriptionManager.getSubscriptionDetails(testData.user, testData.creator);
    ...
    function test_ConcurrentPaymentIntents_Success() public {
        ...
        CommerceProtocolIntegration.PlatformPaymentRequest memory subscriptionRequest = _buildPaymentRequest(subscriptionPayment);
        
        vm.prank(user1);
        (ICommercePaymentsProtocol.TransferIntent memory contentIntent, ) = commerceIntegration.createPaymentIntent(contentRequest);
        
        vm.prank(user1);
        (ICommercePaymentsProtocol.TransferIntent memory subscriptionIntent, ) = commerceIntegration.createPaymentIntent(subscriptionRequest);
