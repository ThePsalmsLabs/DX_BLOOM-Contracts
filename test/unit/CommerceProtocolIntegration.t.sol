...
contract CommerceProtocolIntegrationTest is TestSetup {
    ...
    function test_PaymentIntentCreation_ComprehensiveValidation() public {
        ...
        (ICommercePaymentsProtocol.TransferIntent memory intent, 
         CommerceProtocolIntegration.PaymentContext memory context) = commerceIntegration.createPaymentIntent(request);
    ...
    function test_PaymentExecution_NormalConditions() public {
        ...
        assertTrue(
            afterSnapshot.platformTotalVolume > beforeSnapshot.platformTotalVolume,
            "Platform volume not updated"
        );
    ...
    function test_PaymentExecution_MultiTokenComplexity() public {
        ...
        assertEq(
            initialVolBalance - finalVolBalance, 
            multiTokenScenario.paymentAmount, 
            "VOL tokens not deducted correctly"
        );
        
        // Creator should receive USDC (minus fees)
        assertTrue(
            finalCreatorUSDC > initialCreatorUSDC,
            "Creator did not receive USDC payment"
        );
    ...
    function test_FrontRunningDefense_MEVResistance() public {
        ...
        vm.prank(userScenario.user);
        (ICommercePaymentsProtocol.TransferIntent memory userIntent, ) = commerceIntegration.createPaymentIntent(userRequest);
        ...
        vm.prank(mevBot);
        (ICommercePaymentsProtocol.TransferIntent memory mevIntent, ) = commerceIntegration.createPaymentIntent(mevRequest);
        ...
        CommerceProtocolIntegration.PaymentContext memory userContext = commerceIntegration.getPaymentContext(userIntent.id);
        CommerceProtocolIntegration.PaymentContext memory mevContext = commerceIntegration.getPaymentContext(mevIntent.id);
    ...
    function test_PaymentFailureRecovery_GracefulDegradation() public {
        ...
        (bytes16 originalIntentId, address user, uint256 amount, string memory reason, uint256 requestTime, bool processed) = 
    ...
    function test_SystemLoad_StressConditions() public {
        ...
        for (uint i = 0; i < concurrentPayments; i++) {
        ...
        for (uint i = 0; i < concurrentPayments; i++) {
            ...
            // Check uniqueness
            for (uint j = i + 1; j < concurrentPayments; j++) {
     ...
     */
    function _signIntentHash(bytes32 intentHash, uint256 privateKey) 
        private 
        pure 
        ...
        returns (FinancialSnapshot memory snapshot) 
    {
        ...
        (uint256 totalContent, uint256 activeContent, uint256[] memory categoryCounts, uint256[] memory activeCategoryCounts) = contentRegistry.getPlatformStats();
