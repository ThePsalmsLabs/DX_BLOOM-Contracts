...
contract PayPerViewTest is TestSetup {
    ...
    event ContentPurchaseInitiated(uint256 indexed contentId, address indexed buyer, address indexed creator, bytes16 intentId, PayPerView.PaymentMethod paymentMethod, uint256 usdcPrice, uint256 expectedPaymentAmount);
    event ContentPurchaseCompleted(uint256 indexed contentId, address indexed buyer, address indexed creator, bytes16 intentId, uint256 usdcPrice, uint256 actualAmountPaid, address paymentToken);
    event DirectPurchaseCompleted(uint256 indexed contentId, address indexed buyer, address indexed creator, uint256 price, uint256 platformFee, uint256 creatorEarning);
    ...
    function test_PurchaseContentDirect_Success() public {
        ...
        emit DirectPurchaseCompleted(testContentId1, user1, creator1, DEFAULT_CONTENT_PRICE, platformFee, creatorEarning);
    ...
    function test_CreatePurchaseIntent_USDC() public {
        ...
        emit ContentPurchaseInitiated(testContentId1, user1, creator1, bytes16(0), paymentMethod, DEFAULT_CONTENT_PRICE, DEFAULT_CONTENT_PRICE);
        
        (bytes16 intentId, uint256 expectedAmount, uint256 deadline) = payPerView.createPurchaseIntent(
            testContentId1,
            paymentMethod,
            paymentToken,
            maxSlippage
    ...
    function test_CompletePurchase_Success() public {
        ...
        vm.startPrank(user1);
        (bytes16 intentId, uint256 expectedAmount, ) = payPerView.createPurchaseIntent(
            testContentId1,
            PayPerView.PaymentMethod.USDC,
            address(0),
            100
        ...
        vm.expectEmit(true, true, true, true);
        emit ContentPurchaseCompleted(testContentId1, user1, creator1, intentId, DEFAULT_CONTENT_PRICE, expectedAmount, address(mockUSDC));
    ...
    function test_GetPaymentOptions_Success() public {
        ...
        (PayPerView.PaymentMethod[] memory methods, uint256[] memory prices) = payPerView.getPaymentOptions(testContentId1);
    ...
    function test_RecordExternalPurchase_InactiveContent() public {
        ...
        // Act & Assert: Expect the function to revert
        vm.expectRevert("Content not active");
        payPerView.recordExternalPurchase(testContentId1, user1, bytes16(0), DEFAULT_CONTENT_PRICE, address(mockUSDC), DEFAULT_CONTENT_PRICE);
    }
