...
contract ContentRegistry is Ownable, AccessControl, ReentrancyGuard, Pausable, ISharedTypes {
    ...
    function registerContent(
        ...
        emit ContentRegistered(contentId, msg.sender, ipfsHash, title, category, payPerViewPrice, block.timestamp);
     ...
     */
    function updateContent(uint256 contentId, uint256 newPrice, bool isActive) external nonReentrant whenNotPaused {
     ...
     */
    function recordPurchase(uint256 contentId, address buyer) external onlyRole(PURCHASE_RECORDER_ROLE) {
        if (contentId == 0 || contentId >= nextContentId) revert InvalidContentId();
        ...
        emit ContentPurchased(contentId, buyer, contents[contentId].payPerViewPrice, block.timestamp);
    }
     ...
     */
    function reportContent(uint256 contentId, string memory reason) external nonReentrant whenNotPaused {
        if (contentId == 0 || contentId >= nextContentId) revert InvalidContentId();
        if (!contents[contentId].isActive) revert ContentNotActive();
     ...
     */
    function resolveReport(uint256 contentId, uint256 reportIndex, string memory action)
        external
     ...
     */
    function deactivateContent(uint256 contentId, string memory reason) external onlyRole(MODERATOR_ROLE) {
     ...
     */
    function updateModerationSettings(uint256 newThreshold, uint256 newMaxReports) external onlyRole(MODERATOR_ROLE) {
     ...
     * @return uint256[] Array of content IDs
     */
    function getContentByCategory(ContentCategory category) external view returns (uint256[] memory) {
     ...
     * @return uint256[] Array of active content IDs
     */
    function getActiveContentByCategory(ContentCategory category) external view returns (uint256[] memory) {
     ...
     */
    function getContentReports(uint256 contentId) external view returns (ContentReport[] memory) {
    ...
    function _validateContentData(
        ...
        if (payPerViewPrice < MIN_PAY_PER_VIEW_PRICE || payPerViewPrice > MAX_PAY_PER_VIEW_PRICE) {
            revert InvalidPrice();
        }
     ...
     */
    function _checkContentModeration(string memory title, string memory description, string[] memory tags)
        internal
        view
    {
        // Check title
