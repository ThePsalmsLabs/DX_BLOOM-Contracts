...
abstract contract TestSetup is Test, TestConstants, ISharedTypes {
    ...
    function _deployCoreContracts() internal {
        ...
        refundManager = new RefundManager(address(payPerView), address(subscriptionManager), address(mockUSDC));
        console.log("- RefundManager deployed at:", address(refundManager));

        // Deploy PermitPaymentManager
        permitPaymentManager = new PermitPaymentManager(address(mockCommerceProtocol), address(mockPermit2), address(mockUSDC));
        console.log("- PermitPaymentManager deployed at:", address(permitPaymentManager));

        // Deploy CommerceProtocolIntegration (with all manager addresses)
    ...
    function _configureContracts() internal {
        ...
        // Configure AdminManager addresses
